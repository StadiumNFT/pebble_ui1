...
export async function initViewer(mods){
  ...
  // Soft ground to catch shadows (very light)
  const ground = new THREE.Mesh(
    new THREE.PlaneGeometry(10,10),
    new THREE.MeshStandardMaterial({color:0xf4f8ff, transparent:true, opacity:0.7})
  );
...
export async function sayLine(text){
  try{ isTalking = true; await ttsSpeak(text); }
